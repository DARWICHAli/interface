...
def show_caption(model, features, proba_cls, max_detection=50, beam_size=5, max_len=20, out_size=5):
    ...
    for s, p in ret:
        st.write(f"{s}")  # TODO faire un affichage stat des "p" élevé à l'exponentielle
...
def main():
    ...
    if uploaded_image is not None:
        with st.expander("Parameters of the caption"):
            beam_size = st.slider('Size of the beam search', 1, 20, 5)
            out_size = st.slider('How many caption do you want ?', 1, 5, 1)
            min_boxes = st.slider('Minimum numbers of boxes generated by the segmentation', 3, 40, 10)
            max_boxes = st.slider('Maximum numbers of boxes generated by the segmentation', min_boxes, 100, min_boxes)
            max_len = st.slider('maximum length of the caption', 1, 25,
        ...
        if caption_btn:
            ...
            st.subheader("Caption(s) of this image")
            show_caption(model=cap_model, features=features, proba_cls=proba_cls, max_detection=100,
                         beam_size=beam_size, max_len=max_len, out_size=out_size)
