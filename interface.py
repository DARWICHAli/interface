...
if 'model_dict' not in st.session_state:
    st.session_state.model_dict = md.get_trained_models()
    st.session_state.model_np = []
    for k in st.session_state.model_dict:
        st.session_state.model_np.append(k)
...
@st.cache(allow_output_mutation=True)
def load_both_model(dic):
    return md.load_models_from_dict(dic, "./trained_model")

#st.write(st.session_state)

models_dict = {}
for key in st.session_state.model_dict:
    seg, cap = load_both_model(st.session_state.model_dict[key])
    models_dict[key + "_seg"] = seg
    models_dict[key + "_cap"] = cap
...
# premier model
def apply_segmentation_model(model, image, min_boxes, max_boxes):
    boxes, features, proba_cls, instances = md.segment_picture(model, image, MIN_BOXES=min_boxes,
                                                               MAX_BOXES=max_boxes)
    return boxes, features, proba_cls, instances


def show_boxes(model, original_image, cv2_image, column, min_boxes, max_boxes):
    boxes, features, proba_cls, instances = apply_segmentation_model(model, cv2_image, min_boxes, max_boxes)
...
def show_caption(model, features, proba_cls, max_detection=50, beam_size=5, max_len=20, out_size=5, place=st):
    ret = md.caption_segmentation(model, features, proba_cls, max_detections=max_detection, beam_size=beam_size,
                                  max_len=max_len, out_size=out_size)
    for s, p in ret:
        place.markdown(f"<i><h3>{s}</h3></i>", unsafe_allow_html=True)
    return ret


def show_statistics(place, cap, model):
    words = cap[0]
    words = words.split()
    words = [word + "_" + str(i) for (i, word) in enumerate(words)]
    if model != "R152-M2-256m+cider":
        words.append("<EOS>")

    probas = [i for i in cap[1] if i != 0]
    probas = list(map(math.exp, probas))[:len(words)]
    st.write(len(words), len(probas))
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Parameters", "Segmentation and caption", "Statistics on the caption", "Translate"])

    if uploaded_image is not None:
        tab1.write("Parameters of the caption")
        model_choice = tab1.selectbox("Choose the model you want to use", st.session_state.model_np)
        ...
        seg_model = models_dict[model_choice+"_seg"]
        cap_model = models_dict[model_choice+"_cap"]
        ...
        if caption_btn:
            ...
            boxes, features, proba_cls, instances = show_boxes(seg_model, original_image, img, col2, min_boxes, max_boxes)
            st.session_state.cap = show_caption(model=cap_model, features=features, proba_cls=proba_cls,
                                                max_detection=100,
            ...
            show_statistics(tab3, st.session_state.cap[0], model_choice)
        # tab4
        if st.session_state.cap is not None:
            tab4.subheader("How do you want to translate your caption ? ")
            trad = tab4.radio("In the style of a well known person, or in an other language",
                              ('Celebrity/Gender', 'Language'))
            param = ""
